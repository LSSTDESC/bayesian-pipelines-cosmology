...
from .utils import get_spectra

EPS = 1e-20
J = np.complex(0., 1.)

class FlatSkyMap:
    def __init__(self, N_Z_BINS, N_grid, theta_max, n_z, probe_list, cosmo_pars, shifts, var_gauss, precalculated):
        """
    ...
    def _cl2clg_single_bin(self, Cl, shift1, shift2=None):
        chi = 1./(4. * np.pi) * np.sum(self.Pl_theta * Cl[:self.lmax] * (1 + 2. * self.ls[:self.lmax]), axis=1)
        if shift2 is None:
            shift2 = shift11
        chi_g = np.log(1. + chi / shift1 / shift2)
        Cl_g = (2. * np.pi) * np.sum(((self.w_i * chi_g).reshape(-1,1) * self.Pl_theta), axis=0)
        Cl_g[:2] = 1e-15
        return Cl_g
    ...
    def set_Cl_arr(self, Cl, beta=1.):    
        Cl_arr_real, Cl_arr_imag , inv_Cl_arr_real, inv_Cl_arr_imag = self.get_Cl_arr(Cl, beta)
