...
class MapTools:
  ...
  def do_fwd_KS(self, kappa_l):
    ...
    F_gamma_1 = (self.ell_x**2 -
                 self.ell_y**2) * kappa_l_complex / (self.ell**2 + EPS)
...
# ================== 1D array to Fourier plane ===============================
  ...
  def fourier_plane2array(self, Fx):
    ...
    x = jax.ops.index_update(x, jax.ops.index[:N**2 - 2 * N],
                             Fx[:, :, 1:-1].reshape(-1))
    x = jax.ops.index_update(
        x, jax.ops.index[N**2 - 2 * N:N**2 - 2 * N + 2 * N_edge],
        Fx[:, 1:N_Y - 1, 0].reshape(-1))
    x = jax.ops.index_update(x, jax.ops.index[N**2 - 2 * N + 2 * N_edge:-3],
                             Fx[:, 1:N_Y - 1, -1].reshape(-1))
...
# ================== POST-PROCESSING ===============================
  ...
  def binned_Cl(self, kappa_ell_1, kappa_ell_2, ell_bins):
    ...
    for i in range(len(ell_bins) - 1):
      select_ell = (self.ell > ell_bins[i]) & (
          self.ell < ell_bins[i + 1]) & self.fourier_symm_mask
      ...
      cross_Cl = 2. * np.mean(kappa_ell_1[:, select_ell] *
                              kappa_ell_2[:, select_ell]) / self.Omega_s
